...
def deconvolution_handler(X_sc, X_bulk, TRS, cluster_lists_of_idx, args, param):
    ...
    X_deconvoluted = tunning_weights * X_deconvoluted_unadjusted
...
def optimization_2(X_sc_ct_avg_pos, X_bulk_avg, weights, args, param):  # X是采样数据，n=3，y是bulk数据
...
def loss_3(X_deconvoluted_ct, X_sc_ct, X_ct_avg_i, eta_1=1e-2, eta_2=1e-2):
...
def optimization_3(X_ct_avg, X_sc, clusterIndexList, args, param):  # X是采样数据，n=3，y是bulk数据
    ...
    for i, ct_idx in enumerate(clusterIndexList):
        ...
        # TBD : the above code line has been replaced by the subsequence lines
#        for i, col in enumerate(ct_idx):
        
        with torch.no_grad():
            X_deconvoluted[:,ct_idx] = torch.from_numpy(X_deconvoluted_ct[:,np.arange(len(ct_idx))])
#            X_deconvoluted[:,col] = torch.from_numpy(X_deconvoluted_ct[:,i])
